...
public class CalcServer_1 {
    ...
    while (true) {
      ...
      try {
        if (countOperator(expression) != 1) {
          ...
          double answer = CalcServer_1.getAnswer(expression);
  ...
  static int countOperator(String expression) {
    return (int) expression.codePoints().filter(CalcServer_1::isOperator).count();
  }

  static boolean isFirstAndLastNumber(String expression) {
    if (expression.charAt(0) >= 48 && expression.charAt(expression.length() - 1) <= 57) {
      return true;
    }
    return false;
  }
  ...
  static boolean hasWrongChar(String expression) {
    return expression.codePoints().filter(e -> !isOperator(e)).filter(e -> e < 48 || e > 57)
        .filter(e -> e != 46).count() > 0 ? true : false;
  }
  ...
  static boolean isAnswerInt(String[] numbersArray, String operator) {
    if (operator.equals("+")) {
      return Math.floor(Double.parseDouble(numbersArray[0]))
          + Math.floor(Double.parseDouble(numbersArray[1])) == Math
              .floor(Double.parseDouble(numbersArray[0]) + Double.parseDouble(numbersArray[1]));
